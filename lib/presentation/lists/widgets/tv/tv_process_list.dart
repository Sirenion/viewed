...
class _TvInProcessList extends StatelessWidget {
  const _TvInProcessList({required this.route});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TvCubit, TvState>(
      buildWhen: (previous, current) =>
          ...
          previous.inProcessSort != current.inProcessSort,
      builder: (context, state) {
        ...
        return Column(
          children: [
            SortFilterRow(
              onSortChanged: (value) => context.read<TvCubit>().sort(value, 'inProcess'),
              onFilterPressed: () {},
              selectedSort: state.inProcessSort,
                    ...
                    return ExpansionPanelRadio(
                      ...
                      body: ProcessTileBody(
                        ...
                        decreaseEpisode: () => switch (state.isLocalLoading) {
                          false => context.read<TvCubit>().addEpisode(item, false),
                          true => null,
                        },
                        addEpisode: () => switch (state.isLocalLoading) {
                          false => context.read<TvCubit>().addEpisode(item, true),
                          true => null,
                        },
                        decreaseSeason: () => switch (state.isLocalLoading) {
                          false => context.read<TvCubit>().addSeason(item, false),
                          true => null,
                        },
                        addSeason: () => switch (state.isLocalLoading) {
                          false => context.read<TvCubit>().addSeason(item, true),
                          true => null,
                        },
